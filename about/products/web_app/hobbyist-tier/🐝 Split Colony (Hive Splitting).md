...
#### Performance
- Checkbox selection responds instantly (<100ms)
...
1. **React.memo** with custom comparator prevents re-rendering unaffected frames
2. **useMemo** for onClick handler map ensures stable function references
3. **flushSync** forces immediate DOM updates for instant visual feedback
4. **GPU-accelerated CSS** with `will-change` and `transform: translateZ(0)`
5. **Throttling** (100ms debounce) prevents accidental rapid double-clicks
6. **Lazy image loading** defers loading off-screen images
7. **Resized thumbnails** (smallest available) reduce bandwidth
...
### 📊 Success Metrics
- Split operation completion rate >95%
- Frame selection responsiveness <100ms per click
- Average time to complete split <2 minutes
- Zero data loss during frame migration
- Parent-child relationship accuracy 100%
...
4. **Select Frames to Move**
   - Frames are grouped by box (Box 1, Box 2, etc.)
   - Each frame shows left and right side thumbnail images
   - Click on frames to select/deselect (1-10 frames)
   - Selected frames highlight in yellow
   - Counter shows X/10 frames selected
...
- No automatic queen cell detection/recommendation
...
- Implementation Guide - See `HIVE_SPLIT_IMPLEMENTATION.md` in web-app repository
- Performance Optimization - See `CHECKBOX_PERFORMANCE_OPTIMIZATION.md` in repository root
- [DB Schema](../../../../docs/📱%20Web-app/🥞%20DB%20schemas/swarm-api.md)
